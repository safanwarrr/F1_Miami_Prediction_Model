...
for _, row in results_df.iterrows():
    print(f"{row['driver_name']:<18} | {row['team']:<13} | {row['grid_position']:4.0f} | "
          f"{row['actual_position']:6.0f} | {round(row['pred_linear_reg']):4.0f} | "
          f"{round(row['pred_weighted_avg']):4.0f} | {round(row['pred_random_forest']):4.0f} |")

# Create display version with rounded predictions for end users
results_display = results_df.copy()
results_display['pred_linear_reg_display'] = results_display['pred_linear_reg'].round().astype(int)
results_display['pred_weighted_avg_display'] = results_display['pred_weighted_avg'].round().astype(int)
results_display['pred_random_forest_display'] = results_display['pred_random_forest'].round().astype(int)

# Save both versions: original with floats for model evaluation, display version with integers
results_df.to_csv('miami_gp_predictions_comparison.csv', index=False)
results_display.to_csv('miami_gp_predictions_display.csv', index=False)
print(f"\nDetailed predictions saved to 'miami_gp_predictions_comparison.csv'")
print(f"Display-friendly predictions saved to 'miami_gp_predictions_display.csv'")
