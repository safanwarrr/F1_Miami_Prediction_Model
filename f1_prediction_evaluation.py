...
for model_name, pred_col in models.items():
    ...
    for idx, (_, row) in enumerate(model_predictions.iterrows(), 1):
        print(f"{idx:<4} | {row['driver_name']:<18} | {row['team']:<13} | P{round(row[pred_col]):<8.0f}")
...
for model_name, pred_col in models.items():
    ...
    for _, row in biggest_errors.iterrows():
        print(f"{row['driver_name']:<18} | P{row['actual_position']:<6.0f} | "
              f"P{round(row[pred_col]):<9.0f} | {row['error']:<6.1f}")
